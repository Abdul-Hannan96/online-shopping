...
# Wrap long title
wrapped_title <- str_wrap("👥 Visitors Who Return Are Significantly More Likely to Make a Purchase", width = 50)
...
ggplot(data, aes(x = ExitRates, y = factor(Month), fill = ..x..)) +
  ...
  labs(
    x = "Exit Rate",
    y = "Month",
    title = str_wrap("📆 Seasonal Patterns in Exit Rates", width = 50),
    fill = "Exit Rate"
  ) +
  theme_minimal(base_family = "Georgia") +
  theme(
...
ggplot(data, aes(x = Weekend, y = BounceRates, fill = Weekend)) +
  ...
  labs(
    x = "Is Weekend?",
    y = "Bounce Rate",
    title = "🌦️ Bounce Rate Patterns: Weekend vs Weekday"
  ) +
  theme_minimal(base_family = "Georgia") +
  theme(
...
# Convert durations from seconds to minutes and prepare hover text
data_clean <- data %>%
  mutate(
...
# Create formatted hover text for better readability
data_clean <- data_clean %>%
  mutate(
    product_hours = ProductRelated_Minutes %/% 60,
    product_mins = round(ProductRelated_Minutes %% 60),
    admin_hours = Administrative_Minutes %/% 60,
    admin_mins = round(Administrative_Minutes %% 60),
    
    hover_text = paste0(
      "Product Duration: ", product_hours, "h ", product_mins, "m", "<br>",
      "Administrative Duration: ", admin_hours, "h ", admin_mins, "m", "<br>",
...
p <- ggplot(data_clean, aes(
  x = ProductRelated_Minutes, 
  y = Administrative_Minutes, 
  color = TrafficType,
  text = hover_text   # Pass hover text here
)) +
  ...
  labs(
    ...
    color = "Traffic Type",
    title = "⏱️ Time Spent by Traffic Source"
  ) +
  theme_minimal(base_family = "Georgia") +
  theme(
    panel.background = element_rect(fill = "#f9f9f9", color = NA),
...
# Render with Plotly and show only our custom text in tooltip
